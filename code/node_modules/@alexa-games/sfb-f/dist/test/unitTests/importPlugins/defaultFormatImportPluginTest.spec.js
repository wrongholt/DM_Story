...
>>          return
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.RETURN, `Failed to pass syntax:\n${testString}`);
                }
            }
        }
    });
...
>>          restart
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.RESTART, `Failed to pass syntax:\n${testString}`);
                }
            }
        }
    });
...
>>          RESUME
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.GO_TO, `Failed to pass syntax:\n${testString}`);
...
>>          REPEAT
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.REPEAT, `Failed to pass syntax:\n${testString}`);
                }
            }
        }
    });
...
>>          REPROMPT
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.REPEAT_REPROMPT, `Failed to pass syntax:\n${testString}`);
                }
            }
        }
    });
...
>>          BACK
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.BACK, `Failed to pass syntax:\n${testString}`);
...
>>          PAUSE
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.PAUSE, `Failed to pass syntax:\n${testString}`);
                }
            }
        }
    });
...
>>          END
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.END, `Failed to pass syntax:\n${testString}`);
                }
            }
        }
    });
...
bgm          'some.url.com'
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.BGM, `Failed to pass syntax:\n${testString}`);
...
clear          variable
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.CLEAR, `Failed to pass syntax:\n${testString}`);
...
unflag          variable
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.UNFLAG, `Failed to pass syntax:\n${testString}`);
                    assert_1.strict.equal(startDirections[0].parameters.variableName, 'variable', `Failed to pass syntax:\n${testString}`);
                }
            }
        }
...
flag          variable
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.FLAG, `Failed to pass syntax:\n${testString}`);
                    assert_1.strict.equal(startDirections[0].parameters.variableName, 'variable', `Failed to pass syntax:\n${testString}`);
                }
            }
        }
...
slot          variable as 'slottype'
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.SLOT, `Failed to pass syntax:\n${testString}`);
                    assert_1.strict.equal(startDirections[0].parameters.variableName, 'variable', `Failed to pass syntax:\n${testString}`);
...
set          variable as 'string'
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.SET, `Failed to pass syntax:\n${testString}`);
                    assert_1.strict.equal(startDirections[0].parameters.variableName, 'variable', `Failed to pass syntax:\n${testString}`);
...
increase          variable by 32
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.INCREASE, `Failed to pass syntax:\n${testString}`);
                    assert_1.strict.equal(startDirections[0].parameters.variableName, 'variable', `Failed to pass syntax:\n${testString}`);
                    assert_1.strict.equal(startDirections[0].parameters.variableValue, '32', `Failed to pass syntax:\n${testString}`);
                }
            }
...
decrease          variable by 32
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.REDUCE, `Failed to pass syntax:\n${testString}`);
                    assert_1.strict.equal(startDirections[0].parameters.variableName, 'variable', `Failed to pass syntax:\n${testString}`);
                    assert_1.strict.equal(startDirections[0].parameters.variableValue, '32', `Failed to pass syntax:\n${testString}`);
                }
            }
...
multiply          variable by 32
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.MULTIPLY, `Failed to pass syntax:\n${testString}`);
                    assert_1.strict.equal(startDirections[0].parameters.variableName, 'variable', `Failed to pass syntax:\n${testString}`);
                    assert_1.strict.equal(startDirections[0].parameters.variableValue, '32', `Failed to pass syntax:\n${testString}`);
                }
            }
...
divide          variable by 32
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.DIVIDE, `Failed to pass syntax:\n${testString}`);
                    assert_1.strict.equal(startDirections[0].parameters.variableName, 'variable', `Failed to pass syntax:\n${testString}`);
                    assert_1.strict.equal(startDirections[0].parameters.variableValue, '32', `Failed to pass syntax:\n${testString}`);
                }
            }
...
custom          param = '{othervar}'
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                const startDirections = importResult.importedScenes[0].contents[0].sceneDirections;
                if (startDirections) {
                    assert_1.strict.equal(startDirections[0].directionType, interfaces_1.SceneDirectionType.CUSTOM, `Failed to pass syntax:\n${testString}`);
...
describe("ABC Format Import Plugin Commands Test", function () {
    ...
    it("Update Go To Command Base", async function () {
        const updateGotoCommand = new defaultPlugin.UpdateGotoCommand({
            ...
            goto: "start",
            action: interfaces_1.SceneDirectionType.GO_TO
        });
