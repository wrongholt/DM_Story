...
class StoryStateHelper {
    ...
    static addChoiceHistory(storyState, choiceMade, stateDiff, historySize = 15) {
        ...
        if (currentSceneID == null) {
            throw "[ERROR] Cannot make choice from a 'null' scene.";
        }
    ...
    static getChoiceByUserInput(storyState, userInput, _locale) {
        ...
        if (storyState.system_branch) {
            ...
            if (!fuzziedMatch && userInput.intent && userInput.intent.length > 0) {
                if (BuiltInIntents_1.BUILT_IN_INTENT_UTTERANCES[locale][userInput.intent]) {
                    for (let utterance of BuiltInIntents_1.BUILT_IN_INTENT_UTTERANCES[locale][userInput.intent]) {
                        fuzziedMatch = sfb_util_1.pickBestResponse(utterance, Object.keys(utterance2Choice));
                        if (fuzziedMatch) {
                            break;
                        }
                    }
