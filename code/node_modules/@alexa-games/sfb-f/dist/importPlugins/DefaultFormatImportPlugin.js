...
const SceneDirectionBuilder_1 = require("../aceEntities/SceneDirectionBuilder");
...
function getFormattedStringForAction(action, gotoId) {
    let val = "";
    if (action === interfaces_1.SceneDirectionType.SAVE_AND_GO) {
        val += "<-> " + gotoId;
    }
    else if (action === interfaces_1.SceneDirectionType.GO_TO) {
        val += "-> " + gotoId;
    }
    else if (action === interfaces_1.SceneDirectionType.RESTART) {
        val += ">> RESTART";
    }
    else if (action === interfaces_1.SceneDirectionType.PAUSE) {
        val += ">> PAUSE";
    }
    else if (action === interfaces_1.SceneDirectionType.RESUME) {
        val += ">> RESUME";
    }
    else if (action === interfaces_1.SceneDirectionType.REPEAT) {
        val += ">> REPEAT";
    }
    else if (action === interfaces_1.SceneDirectionType.REPROMPT) {
        val += ">> REPROMPT";
    }
    else if (action === interfaces_1.SceneDirectionType.BACK) {
        val += ">> BACK";
    }
    else if (action === interfaces_1.SceneDirectionType.END) {
        val += ">> END";
    }
    else if (action === interfaces_1.SceneDirectionType.RETURN) {
        val += ">> RETURN";
    }
    return val;
}
...
function parseScenes(contents, ignoreSyntaxError = false) {
    ...
    for (let content of contents) {
        ...
        while (sceneMatch != null) {
            ...
            let sceneContent = sceneMatch[2].replace(/^[ \t]*?\/\/[\s\S]*?$/gm, "").trim();
            try {
...
function parseContent(originSceneID, contentText, availableSceneIDs, ignoreSyntaxError) {
    ...
    let recapPrompts = "";
    let reprompts = "";
    let visualSceneDirection = undefined;
    let sceneDirections = [];
    while (contentMatch != null) {
        ...
        switch (scenePropertyName.trim()) {
            ...
            case "show": {
                ...
                visualSceneDirection = new SceneDirectionBuilder_1.SceneDirectionBuilder().setVisuals(visualOptions).build();
                break;
            }
            case "then":
            case "do": {
    ...
    let sceneVariations = [];
    let finalSceneInstructions = new SceneDirectionBuilder_1.SceneDirectionBuilder();
...
function parseInstructions(sceneDirectionBody, availableSceneIDs, ignoreSyntaxError) {
    let directionBuilder = new SceneDirectionBuilder_1.SceneDirectionBuilder();
    let lineNumber = 0;
    let errorLines = [];
    const bracketStack = [];
