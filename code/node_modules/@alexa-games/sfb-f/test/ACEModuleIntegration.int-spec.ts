import { ACEContentTester } from "./ACEContentTester";
import {
    StoryMetadata
} from "../aceEntities/interfaces";
import { ACEImporter } from "../importer";
import {
    loadAllContent,
    loadTestResource,
...
} from '../extensions/ACEExtension';
import { crashOnUnhandledRejections } from '@alexa-games/sfb-util';
import { ImportErrorLine } from '../aceEntities/interfaces';
...
describe("Scene Transition Baseline", function () {
    let contentTester: ACEContentTester;

    before(async function () {
        let importer = new ACEImporter(undefined, undefined, [
        ...
        contentTester = new ACEContentTester(importedStory, [
            new GlobalDirectionsExtension([]),
            new SnippetExtension({}),
            new VoiceOverExtension("{{file_name}}")
        ]);
...
describe("Skill Flow Builder Test", function () {
    ...
    it("audio source tag", async function () {
        ...
        await contentTester.assertSpeech(
            "playing audio now: <audio src='https://s3.amazonaws.com/alexa-ml/sounds/sound-library-loud/Instruments/Trumpet_1.mp3' />  Playing same tag, but with a snippet. <trumpets> Thanks for testing. Bye."
        );
    });
    ...
    it("multiple voice tags", async function () {
        ...
        await contentTester.assertSpeech(
            "Audio source test. <audio src='https://s3.amazonaws.com/alexa-ml/sounds/sound-library-loud/Instruments/Trumpet_1.mp3' />  Here's a conversation between a few friends. <amazon:effect name='whispered'>I want to tell you a secret.</amazon:effect> <voice name='Brian'>Okay, I'm listening.</voice><voice name='Kendra'><lang xml:lang='en-US'>Americans say, advertisement, and barbecue.</lang> <lang xml:lang='en-GB'>British people say, advertisement, and barbecue. How do you say it?</lang></voice><voice name='Brian'><lang xml:lang='en-GB'>I say advertisement, and, barbecue.</lang> <lang xml:lang='en-US'>But I can also say it your way. Advertisement, barbecue. See?</lang></voice><voice name='Justin'>Do you say, <phoneme alphabet='ipa' ph='pɪˈkɑːn'>pecan</phoneme>, or, <phoneme alphabet='ipa' ph='ˈpi.kæn'>pecan</phoneme>?</voice><voice name='Ivy'> I want to <w role=amazon:VBD>read</w> a new book, but I've already <w role='amazon:VB'>read</w> everything in the library! That didn't sound right, lets try that again. I want to <w role='amazon:VB'>read</w> a new book, but I've already <w role='amazon:VBD'>read</w> everything in the library! Much better! </voice> Thanks for testing. Bye."
        );
    });
    ...
    it("Pause and resume", async function () {
        ...
        await contentTester.givenUserInput({ value: "pause" });
        await contentTester.assertSpeech(
...
describe("Snippet Test", function () {
    ...
    it("Snippets test", async function () {
        ...
        await contentTester.assertSpeech(
            //"Welcome to the snippet test. Here's some snippets. Trumpets internal amazon sound. <trumpets> Ocean external Ezra sound. <ocean wave> Short pause <pause> Was that short? Long pause <longpause> Was that long? Here's a kid's voice. <kid>Hi, my name is Justin</kid>. Old man voice. <oldman>Hello there. I'm really old.</oldman> This is blank. <blank> Was it blank? <voice name='Matthew'>My name is Matthew <lang xml:lang=\"de-DE\">and I speak in a German accent.</lang> Did I sound German?</voice>  Here's a two second pause. <pause for 2s> Was it two seconds? Here's another trumpet sound effect. <sfx trumpet_1.mp3> Did you hear it? Empty with brackets <empty with brackets> End of skill."

            "Welcome to the snippet test. Here's some snippets. Here's a trumpet snippet. <trumpets> Here's an ocean wave. " +
            "<ocean wave> Here's a short pause <pause> Was that short? Here's a long pause. <longpause> Was that long? " +
            "<kid>Hi, my name is Justin.</kid> <oldman>Hi, I'm an old man now.</oldman> Here's a blank snippet. <blank> Did it break? " +
            "Thanks for testing. Bye."
        );
    });
});
