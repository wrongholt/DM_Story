...
>>          return
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.RETURN, `Failed to pass syntax:\n${testString}`);
                }
            }
        }
    });
...
>>          restart
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.RESTART, `Failed to pass syntax:\n${testString}`);
                }
            }
        }
    });
...
>>          RESUME
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.GO_TO, `Failed to pass syntax:\n${testString}`);
...
>>          REPEAT
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.REPEAT, `Failed to pass syntax:\n${testString}`);
                }
            }
        }
    });
...
>>          REPROMPT
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.REPEAT_REPROMPT, `Failed to pass syntax:\n${testString}`);
                }
            }
        }
    });
...
>>          BACK
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.BACK, `Failed to pass syntax:\n${testString}`);
...
>>          PAUSE
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.PAUSE, `Failed to pass syntax:\n${testString}`);
                }
            }
        }
    });
...
>>          END
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.END, `Failed to pass syntax:\n${testString}`);
                }
            }
        }
    });
...
bgm          'some.url.com'
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.BGM, `Failed to pass syntax:\n${testString}`);
...
clear          variable
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.CLEAR, `Failed to pass syntax:\n${testString}`);
...
unflag          variable
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.UNFLAG, `Failed to pass syntax:\n${testString}`);
                    assert.equal(startDirections[0].parameters.variableName, 'variable', `Failed to pass syntax:\n${testString}`);
                }
            }
        }
...
flag          variable
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.FLAG, `Failed to pass syntax:\n${testString}`);
                    assert.equal(startDirections[0].parameters.variableName, 'variable', `Failed to pass syntax:\n${testString}`);
                }
            }
        }
...
slot          variable as 'slottype'
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.SLOT, `Failed to pass syntax:\n${testString}`);
                    assert.equal(startDirections[0].parameters.variableName, 'variable', `Failed to pass syntax:\n${testString}`);
...
set          variable as 'string'
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.SET, `Failed to pass syntax:\n${testString}`);
                    assert.equal(startDirections[0].parameters.variableName, 'variable', `Failed to pass syntax:\n${testString}`);
...
increase          variable by 32
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.INCREASE, `Failed to pass syntax:\n${testString}`);
                    assert.equal(startDirections[0].parameters.variableName, 'variable', `Failed to pass syntax:\n${testString}`);
                    assert.equal(startDirections[0].parameters.variableValue, '32', `Failed to pass syntax:\n${testString}`);
                }
            }
...
decrease          variable by 32
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.REDUCE, `Failed to pass syntax:\n${testString}`);
                    assert.equal(startDirections[0].parameters.variableName, 'variable', `Failed to pass syntax:\n${testString}`);
                    assert.equal(startDirections[0].parameters.variableValue, '32', `Failed to pass syntax:\n${testString}`);
                }
            }
...
multiply          variable by 32
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.MULTIPLY, `Failed to pass syntax:\n${testString}`);
                    assert.equal(startDirections[0].parameters.variableName, 'variable', `Failed to pass syntax:\n${testString}`);
                    assert.equal(startDirections[0].parameters.variableValue, '32', `Failed to pass syntax:\n${testString}`);
                }
            }
...
divide          variable by 32
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.DIVIDE, `Failed to pass syntax:\n${testString}`);
                    assert.equal(startDirections[0].parameters.variableName, 'variable', `Failed to pass syntax:\n${testString}`);
                    assert.equal(startDirections[0].parameters.variableValue, '32', `Failed to pass syntax:\n${testString}`);
                }
            }
...
custom          param = '{othervar}'
        ...
        for (let testString of testCases) {
            ...
            if (importResult.errors && importResult.errors.length > 0) {
                ...
                if (startDirections) {
                    assert.equal(startDirections[0].directionType, SceneDirectionType.CUSTOM, `Failed to pass syntax:\n${testString}`);
