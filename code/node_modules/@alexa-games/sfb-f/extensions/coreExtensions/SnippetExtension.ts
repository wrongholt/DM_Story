    ...
    async extendSourceContent(sourceHelper: SourceContentHelper): Promise<void> {
        ...
        for (let contentItem of contents) {
            for (let snippetKey of Object.keys(this.snippetMap)) {
                ...
                let snippetKeyRegex: RegExp = new RegExp(`[<\\[][\\s]*?${snippetKey}[\\s]*([^>\\[\\]\\s]*)[\\s]*([^>\\[\\]\\s]*)[\\s]*([^>\\[\\]\\s]*)[\\s]*([^>\\[\\]\\s]*)[\\s]*([^>\\[\\]\\s]*)[\\s]*[>\\]]`, "gi"); // Allow up to 5 substitutions in snippets $1 through $5
                ...
                // Replace until I don't need to replace anymore, or I hit 10 times so I don't inifite loop forever
                for(let i = 0; i < 10; i++) {
